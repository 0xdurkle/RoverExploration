 ...
 * Uses transaction with advisory lock and row-level locking to prevent duplicates
 */
  ...
  try {
    ...
    try {
      ...
      if (existingResult.rows.length > 0) {
        ...
        throw new Error(`User ${userId} already has an active exploration (ID: ${existingResult.rows[0].id})`);
      }
